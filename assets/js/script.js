...
function fetchAndRenderPlaylist(searchValue) {
	...
	var objLocation = countryIdArr.find(function (entry) {
		return entry.country === searchValue;
	});
	if (objLocation) {
	...
	var requestPlaylistUrl = `api/deezer/${searchId}`;

	// requests playlist info from deezer
